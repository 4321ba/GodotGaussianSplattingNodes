...
// Must match projection shader exactly
...
layout(push_constant) restrict readonly uniform PushConstants {
    ...
    float _pad0;
    // --- NEW SUN VARIABLES ---
    vec4 sun_direction; // xyz = direction, w = ambient energy
    vec4 sun_color;     // rgb = color * energy
};
...
void main() {
    ...
    for (uint i = 0; i < num_iterations && shared_t > MIN_FACTOR; ++i) {
        ...
        if (id_local < chunk_size) {
            ...
            data.conic = vec3(0.0);
            data.color_and_metallic = vec4(0.0);
            data.normal_and_roughness = vec4(0.0, 1.0, 0.0, 0.5);
            data.image_pos = vec2(0.0);
            data.depth_data = vec4(INVALID_DEPTH, 0.0, 0.0, 0.0);
        }
        conic_tile[id_local] = data.conic;
        color_tile[id_local] = data.color_and_metallic;
        norm_rough_tile[id_local] = data.normal_and_roughness;
        ...
        for (uint j = 0; pixel_in_bounds && j < chunk_size && t > MIN_ALPHA; ++j) {
            ...
            float alpha = color_met.a * exp(power);
            float next_t = t * (1.0 - alpha);
            
            if (alpha > DEPTH_ALPHA && first_hit_depth == INVALID_DEPTH && (1.0 - next_t) >= depth_capture_alpha) {
                first_hit_depth = splat_depth;
            }

            // --- PBR LIGHTING CALCULATIONS ---
            vec3 albedo = color_met.rgb;
            vec3 normal = normalize(norm_rough.xyz);
            
            // Standard Lambertian Diffuse Math
            float ndotl = max(0,dot(normal, sun_direction.xyz));
            vec3 ambient = albedo * sun_direction.w;
            vec3 diffuse = albedo * sun_color.rgb * ndotl;
            
            vec3 lit_color = ambient + diffuse;

            blended_color += lit_color * alpha * t;
            t = next_t;
        }
